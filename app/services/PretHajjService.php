...
class PretHajjService
{
    public function create(array $data): PretHajj
    {
        $attachmentIds = $data['attachmentIds'] ?? [];
        unset($data['attachmentIds']);

        $demande = PretHajj::create($data);
    ...
    {
        return PretHajj::with(['employee', 'reviewer', 'attachments'])
    ...
    {
        return PretHajj::where('employeeId', $employeeId)
            ->with(['attachments'])
            ->orderBy('submittedAt', 'desc')
            ->paginate($perPage);
    }

    public function findOrFail(string $id): PretHajj
    {
        return PretHajj::with(['employee', 'reviewer', 'attachments'])
            ->findOrFail($id);
    }

    public function update(string $id, array $data): PretHajj
    {
    ...
    public function approve(string $id, ?float $approvedAmount, string $adminId): PretHajj
    {
        $demande = $this->findOrFail($id);
