...
class AideSocialeService
{
    public function create(array $data): AideSociale
    {
        ...
        $demande = AideSociale::create($data);
    ...
    {
        return AideSociale::with(['employee', 'reviewer', 'attachments'])
    ...
    {
        return AideSociale::where('employeeId', $employeeId)
            ->with(['attachments'])
            ->orderBy('submittedAt', 'desc')
            ->paginate($perPage);
    }
    ...
    public function approve(string $id, ?float $approvedAmount, string $adminId): AideSociale
    {
        $demande = $this->findOrFail($id);
